...
const StockInPage = () => {
  ...
  const [filters, setFilters] = useState<{ code?: string; product?: string; supplierId?: number; createdBy?: string; range?: [dayjs.Dayjs, dayjs.Dayjs]; statuses?: number[] }>(
    {
      statuses: [0, 1],
    }
  );
  ...
  const load = async () => {
    try {
      ...
      const list = (res.data || []).map((r: any) => ({ id: r.id, code: r.code, time: new Date(r.receiptTime), supplierName: r.supplierName, supplierId: r.supplierId, needPay: r.needPay, status: r.status as 0|1|2 })) as Receipt[];
      setData(list);
  ...
  const filtered = useMemo(() => {
    ...
    return data.filter((x) => {
      ...
      const okSup = !supplierId || (x.supplierId === supplierId || ((x.supplierName || "").toLowerCase() === (suppliersData?.data?.find((s: any) => s.id === supplierId)?.name || "").toLowerCase()));
    ...
    const run = async () => {
      ...
      for (const batch of chunks) {
        const results = await Promise.all(batch.map(async (id) => {
          try { const res = await receiptsService.detail(id); return { id, data: res.data }; } catch { return { id, data: null }; }
        }));
  ...
  return (
    <section>
      ...
      <Form form={searchForm} layout="vertical" onFinish={(vals) => { setFilters({ ...filters, ...vals }); load(); }} initialValues={filters}>
        <Card className="mb-3" title="Lọc dữ liệu" extra={<Space><Button type="primary" icon={<SearchOutlined />} onClick={() => searchForm.submit()}>Tìm kiếm</Button><Button onClick={() => { searchForm.resetFields(); setFilters({ statuses: [0,1] }); }}>Reset</Button></Space>}>
          <Row gutter={16}>
            <Col span={6}><Form.Item name="code" label="Theo mã phiếu nhập"><Input allowClear placeholder="Theo mã phiếu nhập" /></Form.Item></Col>
            <Col span={6}><Form.Item name="product" label="Theo mã, tên hàng"><Input allowClear placeholder="Theo mã, tên hàng" /></Form.Item></Col>
            <Col span={6}><Form.Item name="supplierId" label="Theo NCC">
              <Select
                allowClear
                showSearch
              ...
              <Form.Item label="Trạng thái" name="statuses">
                <Checkbox.Group style={{ width: "100%" }} onChange={(vals) => setFilters({ ...filters, statuses: vals as number[] })}>
                  <Row gutter={[0, 8]}>
                    ...
                    <Col span={24}><Checkbox value={1}>Đã nhập hàng</Checkbox></Col>
      ...
      <CreateEditStockInDrawer open={open} onClose={() => { setOpen(false); setEditingId(null); }} onChanged={() => load()} receiptId={editingId ?? undefined} />
    </section>
  );
};
...
const ReceiptRowDetail = ({ record, onEdit, onCancelled }: { record: any; onEdit: () => void; onCancelled: () => void }) => {
  ...
  const paymentsRows = React.useMemo(() => {
    ...
    return [{
      ...
      method: methodLabel + (detail.paymentMethod === "transfer" ? ` (${detail.supplierBankAccountNumber || "-"}${detail.supplierBankAccountName ? ` - ${detail.supplierBankAccountName}` : ""}${detail.supplierBankName ? ` - ${detail.supplierBankName}` : ""})` : ""),
              ...
              <div>
                <Table<any>
                  ...
                  columns={[
                    ...
                    { title: "Trạng thái", dataIndex: "status", key: "status", width: 140, render: (t: string) => (<Tag color={t === "Đã thanh toán" ? "green" : "red"}>{t}</Tag>) },
  ...
  React.useEffect(() => {
    const run = async () => {
      try {
        ...
        const d = res.data;
        setRows((d?.items || []).map(i => ({ code: i.productCode || i.productId, name: i.productName || "", quantity: i.quantity, costPrice: i.costPrice, amount: (i.quantity || 0) * (i.costPrice || 0) })));
      } catch {
        setRows([]);
      } finally { setLoading(false); }
    };
