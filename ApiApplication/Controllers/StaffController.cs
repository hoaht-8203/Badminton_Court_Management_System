...
namespace ApiApplication.Controllers
{
    ...
    public class StaffController : ControllerBase
    {
        ...
        public async Task<ActionResult<ApiResponse<List<StaffResponse>>>> GetAll(
            [FromQuery] ListStaffRequest request
        )
        {
        ...
        public async Task<ActionResult<ApiResponse<StaffResponse>>> GetById(int id)
        {
            ...
            if (result == null)
                return NotFound(
                    ApiResponse<StaffResponse>.ErrorResponse(
                        $"Nhân viên với id {id} không tìm thấy"
                    )
                );
            return Ok(
                ApiResponse<StaffResponse>.SuccessResponse(
                    result,
                    "Lấy nhân viên theo id thành công"
                )
            );
        }
