...
public class ReceiptService(ApplicationDbContext context) : IReceiptService
{
    ...
    private async Task CreateInventoryCheckForReceiptAsync(Receipt receipt)
    {
        try
        {
            ...
            var productStocks = await _context.Products.Where(p => productIds.Contains(p.Id))
                .Select(p => new { p.Id, p.Stock }).ToListAsync();
            ...
            var inventoryItems = receipt.Items.Select(item => new InventoryCheckItem
            {
                InventoryCheckId = invCheck.Id,
                ProductId = item.ProductId,
                SystemQuantity = productStocks.First(ps => ps.Id == item.ProductId).Stock - item.Quantity, // Số lượng trước khi nhập
            ...
            await _context.SaveChangesAsync();
            Console.WriteLine($"DEBUG: Inventory check saved successfully with ID {invCheck.Id}, Items count: {inventoryItems.Count}");
        }
        catch (ApiException ex)
        {
            Console.WriteLine($"API ERROR: Failed to create inventory check for receipt {receipt.Code}: {ex.Message}");
        ...
        {
            Console.WriteLine($"ERROR: Failed to create inventory check for receipt {receipt.Code}: {ex.Message}");
