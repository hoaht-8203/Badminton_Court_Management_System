...
namespace ApiApplication.Services.Impl
{
    public class StockOutService : IStockOutService
    {
        ...
        public StockOutService(ApplicationDbContext context, IMapper mapper, IInventoryCardService inventoryCardService)
        {
            _context = context;
            _mapper = mapper;
    ...
    public async Task<List<ListStockOutResponse>> ListAsync(DateTime? from, DateTime? to, int? status)
    {
        ...
        public async Task<DetailStockOutResponse> DetailAsync(int id)
        {
            ...
            return new DetailStockOutResponse
            {
                ...
                Items = stockOut.Items.Select(i => new DetailStockOutItem
                {
        ...
        public async Task<int> CreateAsync(CreateStockOutRequest request)
        {
            ...
            stockOut.OutTime = request.OutTime.Kind == DateTimeKind.Utc
                ? request.OutTime
        ...
        private async Task ProcessStockOutCompletion(StockOut stockOut)
        {
            ...
            var inventoryCheck = new InventoryCheck
            {
                ...
                Note = $"Kiểm kho tự động - Xuất hủy {stockOut.Code}",
        ...
        {
            var lastCode = await _context.StockOuts
                .Where(x => x.Code.StartsWith("XH"))
        ...
        {
            var lastCode = await _context.InventoryChecks
                .Where(x => x.Code.StartsWith("KK"))
                .OrderByDescending(x => x.Code)
                .Select(x => x.Code)
                .FirstOrDefaultAsync();
