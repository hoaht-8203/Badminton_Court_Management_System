...
public class ReceiptService(ApplicationDbContext context) : IReceiptService
{
    ...
    public async Task UpdateAsync(int id, CreateReceiptRequest req)
    {
        ...
        if (r.Status == ReceiptStatus.Completed)
            throw new InvalidOperationException("Phiếu đã hoàn thành không thể sửa");
    ...
    public async Task CompleteAsync(int id)
    {
        ...
        var invCheck = new InventoryCheck
        {
            ...
            Note = $"Hoàn thành phiếu nhập {r.Code}"
        };
    ...
    public async Task CancelAsync(int id)
    {
        var r = await _context.Receipts.FirstOrDefaultAsync(x => x.Id == id)
            ?? throw new KeyNotFoundException("Receipt not found");
        if (r.Status != ReceiptStatus.Draft) throw new InvalidOperationException("Chỉ hủy phiếu tạm");
