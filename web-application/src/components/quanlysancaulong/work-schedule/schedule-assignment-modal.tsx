...
const ScheduleAssignModal: React.FC<ScheduleAssignModalProps> = ({ open, onClose, staff, date, onSave }) => {
  ...
  return (
    <Modal open={open} onCancel={onClose} footer={null} width={480} title={
      ...
      <div style={{ marginBottom: 18 }}>
        <div style={{ fontWeight: 500, marginBottom: 8 }}>Chọn ca làm việc <Button type="link" icon={"+"} /></div>
        <div style={{ display: "flex", gap: 24 }}>
          {shifts.map((shift) => (
            <div key={shift.key} style={{ minWidth: 120 }}>
              <Checkbox
                checked={selectedShifts.includes(shift.key)}
                onChange={e => handleShiftChange(shift.key, e.target.checked)}
              >{shift.label}</Checkbox>
      ...
      <div style={{ marginBottom: 18 }}>
        <div style={{ fontWeight: 500, marginBottom: 8 }}>Lặp lại hàng tuần
        ...
        {repeatWeekly && (
          <>
            <div style={{ display: "flex", gap: 8, marginBottom: 8 }}>
              ...
              <Button type="link" onClick={handleSelectAllDays}>Chọn tất cả</Button>
            </div>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
              ...
              <Checkbox checked={workOnHoliday} onChange={e => setWorkOnHoliday(e.target.checked)}>Làm việc cả ngày lễ tết</Checkbox>
            </div>
          </>
        )}
      </div>
      <div style={{ marginBottom: 18 }}>
        <div style={{ fontWeight: 500, marginBottom: 8 }}>Thêm lịch tương tự cho nhân viên khác
