...
public class SupplierBankAccountsController(ISupplierBankAccountService service) : ControllerBase
{
    ...
    [HttpGet("list")]
    public async Task<ActionResult<ApiResponse<List<SupplierBankAccount>>>> List(
        [FromQuery] int supplierId
    )
    {
        var list = await _service.ListAsync(supplierId);
        return Ok(
            ApiResponse<List<SupplierBankAccount>>.SuccessResponse(
                list,
    ...
    public async Task<ActionResult<ApiResponse<int>>> Create(
        [FromBody] UpsertBankAccountRequest req
    )
    {
        ...
        {
            return BadRequest(
                ApiResponse<int>.ErrorResponse("Số tài khoản đã tồn tại cho nhà cung cấp này")
            );
        }
    ...
    )
    {
        ...
        {
            return BadRequest(
                ApiResponse<string>.ErrorResponse("Số tài khoản đã tồn tại cho nhà cung cấp này")
            );
        }
