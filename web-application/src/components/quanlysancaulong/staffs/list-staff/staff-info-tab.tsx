    ...
    <div style={{ padding: 24 }}>
      <Row gutter={16} align="middle">
        ...
        <Col span={8}>
          <div style={{ marginBottom: 8 }}>
            <span style={{ fontWeight: 500 }}>Mã nhân viên:</span> <span>{`NV${String(staff.id).padStart(6, "0")}`}</span>
          </div>
          <Divider style={{ margin: "8px 0" }} />
          <div style={{ marginBottom: 8 }}>
            <span style={{ fontWeight: 500 }}>Ngày sinh:</span> <span>{staff.dateOfBirth ? dayjs(staff.dateOfBirth).format("DD/MM/YYYY") : "-"}</span>
          </div>
          <Divider style={{ margin: "8px 0" }} />
          <div style={{ marginBottom: 8 }}>
            <span style={{ fontWeight: 500 }}>Số CMND/CCCD:</span> <span>{staff.identificationNumber}</span>
          </div>
        </Col>
        <Col span={8}>
          <div style={{ marginBottom: 8 }}>
            <span style={{ fontWeight: 500 }}>Ngày bắt đầu làm việc:</span>{" "}
            <span>{staff.dateOfJoining ? dayjs(staff.dateOfJoining).format("DD/MM/YYYY") : "-"}</span>
          </div>
          <Divider style={{ margin: "8px 0" }} />
          <div style={{ marginBottom: 8 }}>
            <span style={{ fontWeight: 500 }}>Số điện thoại:</span> <span>{staff.phoneNumber}</span>
          </div>
          <Divider style={{ margin: "8px 0" }} />
          <div style={{ marginBottom: 8 }}>
      ...
      <div style={{ marginTop: 24, display: "flex", gap: 12 }}>
        <Button
          ...
          onClick={() => onEditStaff && onEditStaff(staff)}
        >
          Cập nhật thông tin
        </Button>
        {isActive ? (
          ...
          <Button type="primary" icon={<CheckOutlined />} style={{ background: "#52c41a", borderColor: "#52c41a" }} onClick={handleToggleStatus}>
            Bắt đầu làm việc
          </Button>
        )}
      </div>
