...
public class SupplierBankAccountService(ApplicationDbContext context) : ISupplierBankAccountService
{
    ...
    public async Task<int> CreateAsync(UpsertBankAccountRequest req)
    {
        ...
        if (string.IsNullOrEmpty(req.AccountNumber) || string.IsNullOrEmpty(req.AccountName) || string.IsNullOrEmpty(req.BankName))
        {
            throw new ApiException("Số tài khoản, tên tài khoản và tên ngân hàng là các trường bắt buộc", HttpStatusCode.BadRequest);
        }
    ...
    public async Task UpdateAsync(int id, UpsertBankAccountRequest req)
    {
        ...
        var entity =
            ...
            ?? throw new ApiException("Không tìm thấy tài khoản ngân hàng nhà cung cấp", HttpStatusCode.NotFound);
