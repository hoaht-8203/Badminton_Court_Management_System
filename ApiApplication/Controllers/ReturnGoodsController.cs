...
public class ReturnGoodsController(IReturnGoodsService returnGoodsService) : ControllerBase
{
    ...
    [HttpGet("list")]
    public async Task<ActionResult<ApiResponse<List<ListReturnGoodsResponse>>>> List([FromQuery] ListReturnGoodsRequest request)
    {
        var result = await _returnGoodsService.ListAsync(request.From, request.To, request.Status);
        return Ok(ApiResponse<List<ListReturnGoodsResponse>>.SuccessResponse(result, "Lấy danh sách trả hàng thành công"));
    }
    ...
    public async Task<ActionResult<ApiResponse<DetailReturnGoodsResponse>>> Detail(int id)
    {
        ...
        return Ok(ApiResponse<DetailReturnGoodsResponse>.SuccessResponse(result, "Lấy chi tiết trả hàng thành công"));
    }

    [HttpPost("create")]
    public async Task<ActionResult<ApiResponse<int>>> Create([FromBody] CreateReturnGoodsRequest request)
    {
    ...
    [HttpPut("update/{id}")]
    public async Task<ActionResult<ApiResponse<object?>>> Update(int id, [FromBody] CreateReturnGoodsRequest request)
    {
    ...
    public async Task<ActionResult<ApiResponse<object?>>> Complete(int id)
    {
        ...
        return Ok(ApiResponse<object?>.SuccessResponse(null, "Hoàn thành phiếu trả hàng thành công"));
    }
