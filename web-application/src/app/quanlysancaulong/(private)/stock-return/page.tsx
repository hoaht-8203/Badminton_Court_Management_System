    ...
    <section>
      ...
      <div className="mt-4">
        <Table<ReturnGoodsRow>
          ...
          columns={[
            { title: "Mã trả hàng", dataIndex: "code", key: "code", width: 140, render: (v?: string, record?: any) => {
              if ((record as any)?.isSummaryRow) return <span className="font-bold"></span>;
              return v || "";
            }},
            { title: "Thời gian", dataIndex: "time", key: "time", width: 160, render: (d?: Date, record?: any) => {
              if ((record as any)?.isSummaryRow) return <span className="font-bold"></span>;
              return d ? dayjs(d).format("DD/MM/YYYY HH:mm") : "-";
            }},
            { title: "Nhà cung cấp", dataIndex: "supplierName", key: "supplierName", width: 220, render: (t?: string, record?: any) => {
              if ((record as any)?.isSummaryRow) return <span className="font-bold"></span>;
              return t || "";
            }},
            { title: "NCC cần trả", dataIndex: "needPay", key: "needPay", width: 140, render: (v?: number, record?: any) => {
        ...
        items={[
          {
            ...
            children: (
              <div className="space-y-6">
                {/* Thông tin cơ bản */}
                <div>
                  <h3 className="text-lg font-semibold mb-3">Thông tin phiếu trả hàng</h3>
                      ...
                      <div className="md:col-span-3 text-right space-x-2">
                        ...
                        <Button danger onClick={() => {
                          Modal.confirm({
                            ...
                            onOk: async () => {
                              try { await returnGoodsService.cancel(record.id); message.success("Đã hủy phiếu"); onCancelled(); }
                ...
                <div>
                  ...
                  <div className="space-y-1 text-right">
                    <div>
                      ...
                      <span className="font-semibold ml-2">{methodLabel}</span>
                    </div>
                    <div>
                    ...
                    <div>
                      ...
                      <span className="font-semibold ml-2 text-green-600">{(detail?.supplierPaid ?? 0).toLocaleString()}</span>
                    </div>
                  </div>
                </div>
              </div>
